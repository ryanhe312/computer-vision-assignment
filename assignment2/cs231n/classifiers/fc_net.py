...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers):
            ...
            if self.normalization=='layernorm':
                out, cache['batch%d'%i] = layernorm_forward(out, self.params['gamma%d'%i], self.params['beta%d'%i], self.bn_params[i-1])
            out, cache['relu%d'%i] = relu_forward(out)
            if self.use_dropout:
                out, cache['drop%d'%i] = dropout_forward(out, self.dropout_param)
        ...
        for i in reversed(range(1,self.num_layers)):
            if self.use_dropout:
                dout = dropout_backward(dout, cache['drop%d'%i])
            dout = relu_backward(dout,cache['relu%d'%i])
            if self.normalization=='batchnorm':
                dout, grads['gamma%d'%i],grads['beta%d'%i] = batchnorm_backward(dout, cache['batch%d'%i])
            if self.normalization=='batchnorm':
                dout, grads['gamma%d'%i],grads['beta%d'%i] = layernorm_backward(dout, cache['batch%d'%i])
